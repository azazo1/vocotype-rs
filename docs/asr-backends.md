...
| 词典文本改写 | 支持 | 支持 |
| `hotwords_score` 参数 | 已传入 sherpa, 但 sherpa Paraformer 不使用 | 不支持, 参数无效 |
...
- 讯飞 `EdgeEsr` 是默认后端, 适合需要在录音期间看到实时结果和修正结果的场景.
- `sherpa-onnx` 适合只需要离线分段转写和独立标点模型的场景.
...
`hotwords_score` 是解码阶段对候选路径的加权分数. VocoType 会把该参数传给 sherpa-onnx, 但 sherpa-onnx Paraformer 模型不支持 contextual biasing, 因而不会使用该分数. 讯飞 EdgeEsr 当前没有对应实现, 该参数对它无效.
